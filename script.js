...
function renderProducts(filter='all'){
  ...
  grid.innerHTML=products
    ...
    .slice(0,4)
    .map(p=>`<article class="product-card">
...
function renderCart(){
  ...
  wrap.innerHTML=cart.length
    ? cart.map(i=>`<div class="cart-item"><div><b>${i.name}</b><div>$${i.price}</div></div><div class="qty"><button data-minus="${i.id}">−</button><span>${i.qty}</span><button data-plus="${i.id}">+</button></div></div>`).join('')
...
document.addEventListener('click',e=>{
  ...
  if(t.dataset.plus){
    const i=cart.find(x=>x.id==t.dataset.plus); if(!i) return;
    i.qty++; save();
  }

  if(t.dataset.minus){
    const i=cart.find(x=>x.id==t.dataset.minus); if(!i) return;
    i.qty--; cart=cart.filter(x=>x.qty>0); save();
  }
});
